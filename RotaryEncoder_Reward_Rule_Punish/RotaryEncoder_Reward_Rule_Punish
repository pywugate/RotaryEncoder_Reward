...
//  outputV = abs(Diff)*dL*scale;
  ...
  Prev = counter;
  delay(encoderlimit*1);                  // delay x ms to calculate Diffance
  ...
  if ((millis() - steady) <= 500){
    if (state == punishment){
      give_punish();
    }
  }else if ( (millis() - steady) > 1000  && (millis() - steady) <= 4000){
    digitalWrite(out_punish,LOW);
